...
stacked_frames = deque([np.zeros((30, 45), dtype=np.int) for i in range(frames)], maxlen=4)
...
for global_step in range(args.total_timesteps):
    ...
    if random.random() < epsilon:
        ...
        q_value = q_network.forward(obs.reshape((1,) + obs.shape))
    ...
    if global_step > args.learning_starts and global_step % args.train_frequency == 0:
        ...
        with torch.no_grad():
            q_values_next = q_network.forward(s_next_obses)
            actions_max = torch.argmax(q_values_next, dim=1, keepdim=True)
            q_values_next_target = target_network.forward(s_next_obses)
            target_max = q_values_next_target.gather(1, actions_max.long()).squeeze(1)
